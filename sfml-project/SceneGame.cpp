...
{
    Scene::Draw(window);
    //auto activeView = FRAMEWORK.GetWindow().getView();   // ← **그리기 전에 호출**
    ...
    const auto& data = level->tiles;       // vector<int> 전체 타일
    ...
    for (auto& e : level->entities)
    {
        ...
        if (e["type"] == "Door") {
            ...
            box.setOutlineColor(locked ? sf::Color::Red     // 잠김-문
                : sf::Color::Green); // 열림-문
        }
        window.draw(box);
    }
